...
namespace ESForeign
{
	...
	//二次认证链接不存在
	const TErrorCodeType Error_Link_NotExist = 31;
	//需要进行动态认证
	const TErrorCodeType Error_Login_DDA = 32;
	//二次认证时间有效期内无法请求认证码
	const TErrorCodeType Error_Login_RequestVertificateFailed = 33;
	//请求二次认证的联系方式不正确。
	const TErrorCodeType Error_Login_ErrorContact = 34;
	//修改密码时新旧密码重复
	const TErrorCodeType Error_Login_NewPwdRepeat = 35;
